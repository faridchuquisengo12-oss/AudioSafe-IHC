...
function getLang(){return (typeof currentLang!=='undefined')?currentLang:'es';}

function updateEmailDisplay(val){const d=document.getElementById('loginEmailDisplay');if(!d)return;const L=getLang();const prefixes={es:'👤 Conectado como: ',en:'👤 Signed in as: ',zh:'👤 登录为: '};if(val.length>3){d.textContent=(prefixes[L]||prefixes.es)+val;d.classList.add('show');}else{d.classList.remove('show');}}
...
function enterDash(){
  ...
  if(!nombre){errEl.textContent=T.err_name_required;errEl.style.display='block';nombreEl.focus();return;}
  if(!apellido){errEl.textContent=T.err_lastname_required;errEl.style.display='block';apellidoEl.focus();return;}
  if(!email){errEl.textContent=T.err_email_required;errEl.style.display='block';emailEl.focus();return;}
  if(!email.includes('@')){errEl.textContent=T.err_email_invalid_at;errEl.style.display='block';emailEl.focus();return;}
  const emailParts=email.split('@');
  if(emailParts.length!==2||!emailParts[0]||!emailParts[1]||!emailParts[1].includes('.')){errEl.textContent=T.err_email_invalid_format;errEl.style.display='block';emailEl.focus();return;}
  if(!pass||pass.length<6){errEl.textContent=T.err_password_min;errEl.style.display='block';passEl.focus();return;}
  ...
  const segLabels=T.seg_dash_labels||{ciudadano:'Ciudadano',trabajador:'Trabajador',profesional:'Profesional de salud'};
  const subs=T.seg_dash_subs||{ciudadano:'',trabajador:'',profesional:''};
  ...
  document.getElementById('dashGreet').textContent=(T.dash_welcome_msg||'👋 Bienvenido, ')+nombre;
  ...
  document.getElementById('pfSegmento').value=segLabels[pendingSeg]||'';
...
const i18n={
  es:{
    ...
    add_point_hint:'📍 Haz clic en el mapa para colocar tu punto',
  ...
  en:{
    ...
    add_point_hint:'📍 Click on the map to place your noise point',
  ...
  zh:{
    ...
    dbx_w_lead:'什么是分贝（dB）？',dbx_w_body:'分贝（dB）用来衡量声音强度。SUNAFIL 规定 8 小时工作日内 85 dB 为安全职业暴露上限。',
    dbx_p_lead:'什么是分贝（dB）？',dbx_p_body:'分贝（dB）以对数刻度衡量声音强度：每增加 10 dB，响度大约感知为原来的两倍。世卫组织建议城市地区不超过 53 dB。',
    hero_p:'AudioSafe 实时测量声级，生成协作地图，发送个性化警报，帮助您在利马的城市和工作环境中保护听力健康。',
    ...
    problem_p:'利马主要大道记录到 75-85 dB 的噪声水平，远超 53 dB 推荐限值。听力损伤是无声且累积的。',
    prob_c1_h:'隐形损伤',prob_c1_p:'噪声性听力损失起初不痛，发现较晚，损伤已不可逆。',
    prob_c2_h:'嘈杂的城市',prob_c2_p:'超过 60% 的秘鲁城市人口生活在交通繁忙、噪声危险的区域。',
    prob_c3_h:'职业风险',prob_c3_p:'全球每 3 名工人中就有 1 人在工作期间暴露于超过 85 分贝的噪声中。',
    prob_c4_h:'诊断滞后',prob_c4_p:'医生缺乏患者的历史暴露客观数据来评估实际风险。',
    prob_c5_h:'缺乏可用工具',prob_c5_p:'现有解决方案要么过于专业，要么缺乏个性化警报和协作地图。',
    prob_c6_h:'问题持续增长',prob_c6_p:'到 2050 年，超过 25 亿人将出现不同程度的听力损失（世卫组织，2021 年）。',
    benefit_eyebrow:'优势',benefit_h2:'为每位用户量身定制的体验',
    benefit_p:'AudioSafe 为需要管理声音暴露的市民、工人和健康专业人士提供价值。',
    seg1_h:'城市市民',seg1_p:'长期暴露于城市噪声的 18-60 岁人群。',
    seg1_li1:'通过智能手机实时测量。',seg1_li2:'超过风险阈值时发出警报。',seg1_li3:'日常路线噪声区地图。',seg1_li4:'保护听力的建议。',
    seg1_btn:'查看市民仪表板',
    seg2_h:'噪声区工人',seg2_p:'操作员、机械师、音乐家和持续噪声工作环境中的员工。',
    seg2_li1:'可核查的每日暴露记录。',seg2_li2:'超过 85 分贝限值时发出警报。',seg2_li3:'可导出的医疗/工作历史记录。',seg2_li4:'符合 SUNAFIL/第 29783 号法律规定。',
    seg2_note:'ℹ️ SUNAFIL 是秘鲁负责监督劳动安全的机构；第 29783 号法律规定了工作场所噪音暴露的法定限值。',
    seg2_btn:'查看工人仪表板',
    ...
    feat5_h:'可导出历史记录',feat5_p:'可下载 PDF 格式的暴露记录，供医生、雇主或当局使用。',
    ...
    feat9_h:'多平台访问',feat9_p:'可在网页、iOS 和 Android 上使用。设备间自动同步。',
    how_eyebrow:'如何使用？',how_h2:'4 个简单步骤',
    step1_h:'创建账户',step1_p:'注册并选择您的档案：市民、工人或专业人员。',
    step2_h:'激活麦克风',step2_p:'AudioSafe 使用您的智能手机麦克风捕捉声音环境。',
    step3_h:'实时监测',step3_p:'实时查看噪声级，当超过限值时接收警报。',
    step4_h:'生成报告',step4_p:'以 PDF 格式导出历史记录，供医疗、工作或个人使用。',
    plans_eyebrow:'计划',plans_h2:'选择适合您需求的计划',
    plan1_h:'免费',plan1_p:'开始测量和了解您的暴露情况。',
    plan1_li1:'基本实时测量。',plan1_li2:'简单阈值警报。',plan1_li3:'访问协作地图。',plan1_li4:'最近 7 天的历史记录。',
    plan1_btn:'免费开始',
    plan2_h:'高级版',plan2_p:'适合需要更多控制的市民和工人。',plan2_price:'S/ 12.90',plan2_period:'/ 月',
    plan2_li1:'无限可导出历史记录。',plan2_li2:'高级自定义警报。',plan2_li3:'每日/每周剂量分析。',plan2_li4:'供医生或雇主使用的 PDF 报告。',plan2_li5:'优先支持。',
    plan2_btn:'登录',popular_tag:'最受欢迎',
    plan3_h:'机构版',plan3_p:'适用于诊所、企业和市政府。',plan3_price:'定制',
    plan3_li1:'多用户多站点面板。',plan3_li2:'按地理区域分析报告。',plan3_li3:'与职业卫生系统集成。',plan3_li4:'与 SUNAFIL/市政府签订协议。',plan3_li5:'专属支持。',
    plan3_btn:'申请信息',
    faq_eyebrow:'常见问题',faq_h2:'开始前解答您的疑问',
    faq1_q:'我需要特殊设备来测量噪音吗？',faq1_a:'不需要。AudioSafe 使用智能手机内置麦克风测量声级，无需额外硬件。',
    faq2_q:'测量精度如何？',faq2_a:'与专业声级计相比，测量精度为 ±2-3 dB。对于临床或法律场景，建议补充使用认证设备。',
    faq3_q:'AudioSafe 有助于遵守 SUNAFIL 法规吗？',faq3_a:'是的。可导出的每日暴露记录和可核查的历史记录有助于在检查时证明职业风险监测。',
    faq4_q:'我的位置和健康数据是否私密？',faq4_a:'是的。未经您明确同意，您的个人和位置数据永远不会与第三方共享。协作地图数据以匿名汇总方式共享。',
    faq5_q:'我可以作为健康专业人员为患者使用 AudioSafe 吗？',faq5_a:'是的。专业版包括一个患者面板，具有暴露历史记录、可导出报告和为临床使用设计的纵向分析。',
    faq6_q:'在没有网络的区域能使用吗？',faq6_a:'AudioSafe 可以离线捕获数据，并在恢复连接时同步。协作地图需要连接才能更新其他用户的数据。',
    contact_eyebrow:'联系',contact_heading:'您有任何疑问吗？',contact_p:'我们在这里帮助您。请写信给我们，我们将在 24 小时内回复。',
    contact_talk_h:'联系我们',contact_talk_p:'我们的团队随时准备为您提供平台、机构计划或定制集成方面的指导。',
    contact_hours:'周一至周五：上午 9:00 – 下午 6:00',
    ...
    modal_welcome:'欢迎使用 AudioSafe',modal_sub:'登录以访问仪表板。',
    ...
    dash_greeting:'控制面板',
    btn_stop:'⏹ 停止监测',btn_start:'▶️ 开始监测',btn_report:'📊 导出 CSV',btn_export_pdf:'📄 导出报告（PDF）',btn_share:'📍 分享位置',
    mic_error:'我们无法访问您的麦克风。请在浏览器设置中启用权限。',mic_retry:'🔄 重试',
    kpi_c1:'当前级别',kpi_c2:'今日暴露',kpi_c3:'活跃警报',kpi_c4:'每周剂量',
    kpi_c1_trend:'↑ 超过限值',kpi_c2_sub:'最多 8 小时',kpi_c3_trend:'↑ 今天 1 个新警报',kpi_c4_trend:'↓ 受控',
    chart_today:'历史记录 — 最近 12 小时',tag_today:'今天',
    panel_recent_alerts:'近期警报',
    alert1_p:'检测到临界级别',alert1_s:'95 dB · 哈维尔·普拉多大道 · 2 小时前',
    alert2_p:'中等噪声区域',alert2_s:'72 dB · 米拉弗洛雷斯 · 4 小时前',
    alert3_p:'安全级别',alert3_s:'48 dB · 肯尼迪公园 · 6 小时前',
    panel_map:'区域地图 — 利马',map_legend1:'安全 <65dB',map_legend2:'中等 65-85dB',map_legend3:'临界 >85dB',map_last_update:'最后更新：2 分钟前',
    panel_recs:'建议',
    rec1_p:'使用耳塞',rec1_s:'如果您将在高噪声区域停留超过 1 小时。',
    rec2_p:'进行听力休息',rec2_s:'接下来 2 小时避免使用耳机。',
    rec3_p:'有替代路线',rec3_s:'阿雷基帕大道：平均级别 58 dB。',
    kpi_w1:'当前暴露',kpi_w2:'每日剂量',kpi_w3:'临界区域时间',kpi_w4:'无风险天数',
    kpi_w1_trend:'↑ 超过限值（85 dB）',kpi_w2_trend:'↑ 高风险',kpi_w3_trend:'↑ 最多 8 小时',kpi_w4_trend:'↓ 本周',
    chart_week:'暴露记录 — 本周',days_short:'周一,周二,周三,周四,周五,周六,周日',
    warn_box:'⚠️ 周三和周四超过了限值。建议周末进行听力休息。',
    panel_dose:'累计剂量',dose_label:'每日累计剂量',
    panel_sunafil:'SUNAFIL 历史',tag_verif:'可核查',
    th_date:'日期',th_max:'最高级别',th_dose:'剂量',th_time:'时间',th_status:'状态',th_avg:'平均级别',th_exptime:'暴露时间',
    btn_export_csv:'📊 导出 CSV',
    kpi_p1:'活跃患者',kpi_p2:'听力风险',kpi_p3:'已生成报告',kpi_p4:'区域平均级别',
    kpi_p1_trend:'↑ 本周 3 名新患者',kpi_p2_trend:'↑ 需要关注',kpi_p3_sub:'本月',kpi_p4_trend:'↑ 北部工业区域',
    panel_patients:'患者面板',tag_clinical:'临床',
    th_patient:'患者',th_sector:'部门',th_exposure:'暴露',th_risk:'风险',
    panel_risk_dist:'风险分布',
    riskdist_critical:'临界（>90 dB）',riskdist_high:'高（85–90 dB）',riskdist_moderate:'中等（65–85 dB）',riskdist_safe:'安全（<65 dB）',
    label_patient:'名患者',label_patients:'名患者',
    btn_gen_report:'📊 生成机构报告（CSV）',
    panel_hist_compare:'📈 历史级别对比',tag_6months:'6 个月',
    ...
    hist_compare_caption:'患者每月报告的平均级别（左）与最高级别（右）对比。将鼠标悬停在每个柱子上可查看详细信息和风险级别。',
    tt_avg:'平均',tt_max:'最高',
    tt_risk_normal:'正常',tt_risk_moderate:'中等风险',tt_risk_high:'高风险',
    panel_prevent:'🩺 预防建议',
    prevent1_p:'强制听力保护',prevent1_s:'对于 >85 dB 的暴露，建议使用认证的耳塞或耳机。',
    prevent2_p:'定期听力休息',prevent2_s:'在嘈杂环境中建议每 2 小时休息 15 分钟。',
    prevent3_p:'定期听力测试',prevent3_s:'对反复剂量 >80% 的患者进行年度检查。',
    prevent4_p:'转诊至耳鼻喉科专家',prevent4_s:'临界风险患者应在 7 天内接受评估。',
    toast_updating_map:'🔄 正在更新地图点...',
    map_panel_title:'🗺️ 噪声地图 — 利马大都会',tag_live:'直播',
    btn_refresh:'🔄 刷新',btn_add_point:'📍 添加我的点',btn_export_map:'📥 导出地图',
    add_point_hint:'📍 点击地图放置您的噪声点',
    hist_title:'📁 完整暴露历史',tag_30days:'30 天',
    btn_filter_date:'📅 按日期筛选',btn_clear_filter:'✕ 清除筛选',
    kpi_a1:'今日警报',kpi_a2:'本周',kpi_a3:'临界区域',kpi_a4:'今日最高级别',
    kpi_a1_trend:'↑ 2 个临界',kpi_a2_trend:'↑ 4 个已解决',kpi_a3_trend:'↑ 利马北部',kpi_a4_trend:'↑ 哈维尔·普拉多',
    alerts_center_title:'🔔 警报中心',tag_active:'活跃',
    btn_mark_read:'✓ 全部标记为已读',btn_config_alerts:'⚙️ 配置警报',
    falert1_p:'🚨 临界 — 检测到 96 dB 级别',falert1_s:'哈维尔·普拉多东大道 · 15 分钟前 · 您的当前位置',
    falert2_p:'⚠️ 每日剂量超过 85%',falert2_s:'达到自定义阈值 · 1 小时前',
    falert3_p:'中等区域 — 拉马里纳大道',falert3_s:'74 dB · 3 小时前 · 上班途中',
    falert4_p:'✅ 检测到安全级别 — 肯尼迪公园',falert4_s:'48 dB · 6 小时前',
    btn_resolve:'解决',btn_postpone:'推迟',
    config_alerts_title:'🔔 配置警报',
    cfg_threshold_h:'警报阈值',cfg_threshold_s:'通知声级',
    cfg_push_h:'推送警报',cfg_push_s:'浏览器通知',
    cfg_email_h:'邮件警报',cfg_email_s:'每日邮件摘要',
    cfg_night_h:'夜间静音模式',cfg_night_s:'晚上 10 点至早上 7 点无警报',
    zones_title:'📍 监测区域',
    zone_active:'活跃',zone_moderate:'中等',zone_critical:'临界',
    status_high_risk:'高风险',status_exceeds_limit:'超过限值',status_moderate:'中等',status_safe:'安全',status_critical:'临界',status_high:'高',
    zone_ciu1_n:'📍 家',zone_ciu1_s:'米拉弗洛雷斯，利马 · 58 dB',
    zone_ciu2_n:'💼 工作',zone_ciu2_s:'圣伊西德罗，利马 · 78 dB',
    zone_ciu3_n:'🏋️ 健身房',zone_ciu3_s:'苏尔科，利马 · 91 dB',
    zone_trab1_n:'🏭 生产工厂',zone_trab1_s:'工业区，阿特 · 92 dB',
    zone_trab2_n:'🔧 维修车间',zone_trab2_s:'卡亚俄 · 84 dB',
    zone_trab3_n:'🧰 行政办公室',zone_trab3_s:'卡亚俄 · 58 dB',
    zone_prof1_n:'🏥 耳鼻喉科诊室',zone_prof1_s:'圣费利佩诊所，赫苏斯玛丽亚 · 52 dB',
    zone_prof2_n:'🩺 候诊室',zone_prof2_s:'圣费利佩诊所 · 68 dB',
    zone_prof3_n:'🏭 工业工厂参观',zone_prof3_s:'北部工业区 · 89 dB',
    btn_add_zone:'+ 添加区域',
    prefs_title:'🎛️ 应用偏好',pref_theme:'主题',pref_lang:'语言',pref_unit:'计量单位',
    btn_change_theme:'更改主题',btn_save_prefs:'💾 保存偏好',
    prof_activity_title:'📊 您的活动摘要',prof_sessions:'监测会话',prof_alerts_recv:'收到的警报',prof_reports:'生成的报告',
    ...
    err_password_min:'⚠️ 密码长度必须至少为 6 个字符。',
    seg_dash_labels:{ciudadano:'市民',trabajador:'工人',profesional:'健康专业人员'},
    seg_dash_subs:{ciudadano:'监测您在利马的每日声音暴露',trabajador:'职业暴露记录 — 第 29783 号法律',profesional:'暴露于噪音患者的临床随访'},
    dash_welcome_msg:'👋 欢迎， ',
  },
};

// ── Actualizar la etiqueta "Más completo" / "Most complete" del CSS cuando cambie el idioma ──
(function syncFeaturedLabel(){
  const L=getLang();
  const T=i18n[L]||i18n.es;
  document.documentElement.style.setProperty('--seg-featured-label', JSON.stringify(T.seg_featured||''));
  // Observar cambios de idioma si existe la función setLang
  ...
  if(origSetLang){
    ...
    // Si no existe setLang, observar mutations en el lang-btn o data-lang
