...
function enterDash(){
  ...
  if(!nombre){
    errEl.textContent=t.err_need_name;
    errEl.style.display='block';
    nombreEl.focus();return;
  }
  if(!apellido){
    errEl.textContent=t.err_need_lastname;
    errEl.style.display='block';
    apellidoEl.focus();return;
  }
  if(!email){
    errEl.textContent=t.err_need_email;
    errEl.style.display='block';
    emailEl.focus();return;
  }
  if(!email.includes('@')){
    errEl.textContent=t.err_email_invalid;
    errEl.style.display='block';
    emailEl.focus();return;
  }
  const emailParts=email.split('@');
  if(emailParts.length!==2||!emailParts[0]||!emailParts[1]||!emailParts[1].includes('.')){
    errEl.textContent=t.err_email_format;
    errEl.style.display='block';
    emailEl.focus();return;
  }
  if(!pass||pass.length<6){
    errEl.textContent=t.err_pass_short;
    errEl.style.display='block';
    passEl.focus();return;
  }
  ...
  loggedInFirstName=nombre;
  const fullName=nombre+' '+apellido;
  const segLabels={ciudadano:'Citizen',trabajador:'Worker',profesional:'Health Professional'};
  const subs={ciudadano:t.dash_sub_ciudadano,trabajador:t.dash_sub_trabajador,profesional:t.dash_sub_profesional};
  ...
  document.getElementById('dashGreet').textContent=t.dash_welcome_prefix+nombre;
...
const i18n={
  es:{
    ...
    trust_stat1:'umbral de riesgo laboral',trust_stat2:'monitoreo en tiempo real',trust_stat3:'precisión de medición',trust_stat4:'segmentos especializados',
    trust_aligned:'Alineado con:',trust_logo1:'🏛️ SUNAFIL · Ley 29783',trust_logo2:'🌍 Lineamientos de ruido ambiental de la OMS',trust_logo3:'🏥 Clínicas ORL asociadas',
    testi1_q:'"Camino por La Molina pensando que es tranquilo, pero la app me mostró que hay tramos bastante ruidosos en mi ruta diaria. Ahora los evito cuando puedo."',testi1_role:'ciudadana urbana, La Molina',
    testi2_q:'"En Bshaper, el ruido es parte del día a día. Con AudioSafe por fin tengo un registro real de cuánto me expongo, no solo lo que supongo."',testi2_role:'trabajador en Bshaper, Barranco',
    testi3_q:'"Casi nunca les creían a mis pacientes cuánto ruido soportaban en el trabajo. Ahora llegan con su historial, y eso cambia toda la consulta."',testi3_role:'especialista ORL, Villa María del Triunfo',
    midfunnel_p:'Ya viste cómo funciona. Empieza a medir tu exposición hoy.',
    dash_sub_ciudadano:'Monitorea tu exposición sonora diaria en Lima',dash_sub_trabajador:'Registro de exposición ocupacional — Ley N.° 29783',dash_sub_profesional:'Seguimiento clínico de pacientes expuestos al ruido',
    dash_welcome_prefix:'👋 Bienvenido, ',
    err_need_name:'⚠️ Por favor ingresa tu nombre.',err_need_lastname:'⚠️ Por favor ingresa tu apellido.',err_need_email:'⚠️ Por favor ingresa tu correo electrónico.',err_email_invalid:'⚠️ El correo ingresado no es válido. Debe incluir el símbolo "@" (ejemplo: usuario@correo.com).',err_email_format:'⚠️ El formato del correo no es válido. Ejemplo correcto: usuario@correo.com',err_pass_short:'⚠️ La contraseña debe tener al menos 6 caracteres.',
  ...
  en:{
    ...
    trust_stat1:'occupational risk threshold',trust_stat2:'real-time monitoring',trust_stat3:'measurement accuracy',trust_stat4:'specialized segments',
    trust_aligned:'Aligned with:',trust_logo1:'🏛️ SUNAFIL · Law 29783',trust_logo2:'🌍 WHO environmental noise guidelines',trust_logo3:'🏥 Partner ENT clinics',
    testi1_q:'"I walk around La Molina thinking it\'s quiet, but the app showed me there are pretty noisy stretches on my daily route. Now I avoid them when I can."',testi1_role:'urban citizen, La Molina',
    testi2_q:'"At Bshaper, noise is part of everyday life. With AudioSafe I finally have a real record of how much I\'m exposed to, not just what I assume it is."',testi2_role:'worker at Bshaper, Barranco',
    testi3_q:'"My patients were almost never believed about how much noise they endured at work. Now they come in with their history, and that changes the whole consultation."',testi3_role:'ENT specialist, Villa María del Triunfo',
    midfunnel_p:'You\'ve seen how it works. Start measuring your exposure today.',
    dash_sub_ciudadano:'Monitor your daily sound exposure in Lima',dash_sub_trabajador:'Occupational exposure record — Law No. 29783',dash_sub_profesional:'Clinical follow-up of patients exposed to noise',
    dash_welcome_prefix:'👋 Welcome, ',
    err_need_name:'⚠️ Please enter your first name.',err_need_lastname:'⚠️ Please enter your last name.',err_need_email:'⚠️ Please enter your email address.',err_email_invalid:'⚠️ The email entered is not valid. It must include the "@" symbol (example: user@email.com).',err_email_format:'⚠️ The email format is not valid. Correct example: user@email.com',err_pass_short:'⚠️ The password must be at least 6 characters long.',
  ...
  zh:{
    ...
    trust_stat1:'职业风险阈值',trust_stat2:'实时监测',trust_stat3:'测量精度',trust_stat4:'专属细分',
    trust_aligned:'合作对齐：',trust_logo1:'🏛️ SUNAFIL · 第29783号法律',trust_logo2:'🌍 世卫组织环境噪音指南',trust_logo3:'🏥 合作耳鼻喉诊所',
    testi1_q:'"我在拉莫利纳散步时以为很安静，但应用显示我每天的路线上有相当嘈杂的路段。现在我尽量避开它们。"',testi1_role:'城市居民，拉莫利纳',
    testi2_q:'"在Bshaper，噪音是日常生活的一部分。有了AudioSafe，我终于有了真实的暴露记录，而不仅仅是我的猜测。"',testi2_role:'Bshaper员工，巴兰科',
    testi3_q:'"我的病人几乎从未被相信他们在工作中忍受了多少噪音。现在他们带着记录来看病，这改变了整个问诊过程。"',testi3_role:'耳鼻喉专科医生，比亚利亚',
    midfunnel_p:'您已经了解了它的工作原理。今天就开始测量您的暴露量吧。',
    dash_sub_ciudadano:'监测您在利马的每日声音暴露',dash_sub_trabajador:'职业暴露记录 — 第29783号法律',dash_sub_profesional:'噪音暴露患者的临床随访',
    dash_welcome_prefix:'👋 欢迎，',
    err_need_name:'⚠️ 请输入您的名字。',err_need_lastname:'⚠️ 请输入您的姓氏。',err_need_email:'⚠️ 请输入您的电子邮箱。',err_email_invalid:'⚠️ 输入的邮箱无效。必须包含"@"符号（例如：user@email.com）。',err_email_format:'⚠️ 邮箱格式无效。正确示例：user@email.com',err_pass_short:'⚠️ 密码长度必须至少为6个字符。',
...
function setLang(lang,silent){
  ...
  const modalLabels=document.querySelectorAll('#loginModal .form-group label');
  ...
  // MIDFUNNEL CTA
  const midfunnelP=document.querySelector('.midfunnel-cta p');if(midfunnelP)midfunnelP.textContent=t.midfunnel_p;

  // TRUST BAR
  const trustStats=document.querySelectorAll('.trust-stat span:last-child');
  const trustKeys=['trust_stat1','trust_stat2','trust_stat3','trust_stat4'];
  trustStats.forEach((el,i)=>{if(trustKeys[i])el.textContent=t[trustKeys[i]];});
  const trustLabel=document.querySelector('.trust-label');if(trustLabel)trustLabel.textContent=t.trust_aligned;
  const trustLogos=document.querySelectorAll('.trust-logo');
  const trustLogoKeys=['trust_logo1','trust_logo2','trust_logo3'];
  trustLogos.forEach((el,i)=>{if(trustLogoKeys[i])el.textContent=t[trustLogoKeys[i]];});

  // TESTIMONIALS (se conserva el nombre propio, se traduce el rol/ubicación)
  const testiCards=document.querySelectorAll('.testimonial-card');
  const testiNames=['Cecibel Cárdenas','Juliano Bonino','Fernando Gordillo León'];
  const testiData=[['testi1_q','testi1_role'],['testi2_q','testi2_role'],['testi3_q','testi3_role']];
  testiCards.forEach((card,i)=>{
    if(!testiData[i])return;
    const p=card.querySelector('p');if(p)p.textContent=t[testiData[i][0]];
    const foot=card.querySelector('footer');if(foot)foot.textContent='— '+testiNames[i]+', '+t[testiData[i][1]];
  });
  ...
  if(dg){dg.textContent=loggedInFirstName?(t.dash_welcome_prefix+loggedInFirstName):t.dash_greeting;}
  const dsub=document.getElementById('dashSub');
  if(dsub){const subsNow={ciudadano:t.dash_sub_ciudadano,trabajador:t.dash_sub_trabajador,profesional:t.dash_sub_profesional};if(subsNow[pendingSeg])dsub.textContent=subsNow[pendingSeg];}
